...
#just to see the preference scores next to each other
...
```
   ...
   - none show anything significant. This could be do to the fact that F test assumes normally distributed data. Can try transforming the data (next step), or Levene's test (last step). 
...
```
  - Not really sure if the var test for greater is testing if y is greater than x, or if x is greater than y, so not sure how to interpret. Either way, it's not significant.
...
```
   ...
   * Will now try Levene's test. This is similar to the F-test in the sense that it is comparing variances, but it does not assume normality.
