...
orig2 <- ggplot(rawEX, aes(x=X.U.FEFF.SPP, y=TOT.PROP)) + geom_boxplot(outlier.colour = "red", outlier.shape = 8, outlier.size = 2) + xlab("Species") + ylab("Proportion of left-eye bias") + ggtitle("Original Data") + theme_classic()
...
Outlier(LAT_rawEX$TOT.PROP, method=c("boxplot"), value = FALSE)

Outlier(FORM_rawEX$TOT.PROP, method=c("boxplot"), value = FALSE)
...
rem_out2 <- ggplot(rawextra, aes(x=X.U.FEFF.SPP, y=TOT.PROP)) + geom_boxplot() + xlab("Species") + ylab("Proportion of left-eye bias") + ggtitle("Outlier-Removed Data") + theme_classic()
...
* eye preferences: none for sailfin; 0.025 p for amazon but median falls in CI?
...
```
  ...
  * variance: p is .007, ratio of variance within CI (not sure if that's the same thing?)
