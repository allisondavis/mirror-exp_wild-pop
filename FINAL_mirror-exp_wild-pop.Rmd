...
test <- FORM_rawEX[-c(11),] #if I just remove Gretchen (who so far is the only one who doesn't move for >5min)

```

Variance: still significant (p=0.007, 95% CI 1.39-7.45); without gretchen only, it's not significant

```{r}

var.test(LAT_rawEX$LI.TOT, test$LI.TOT, alternative = "two.sided", conf.level = 0.95)
...
t.test(test$LI.A, test$LI.B, paired = TRUE, alternative = "two.sided")


```


* eye preferences: sailfin still no pref, amazon with pretty much no pref (p=0.055, 95% CI -5.78-0.062)

```{r}

t.test(LAT_rawEX$LI.TOT, mu=0, alternative = "two.sided")

t.test(test$LI.TOT, mu=0, alternative = "two.sided")
...
t.test(LAT_rawEX$LI.TOT, test$LI.TOT)

totLAT <- abs(LAT_rawEX$LI.TOT)
totFORM1 <- abs(test$LI.TOT)

t.test(totLAT, totFORM1)


```

***I will be following the methods described by a number of Bisazza papers in which a laterality index is calculated, and parametric tests are used. I will present a qqplot as well to check normality.***
